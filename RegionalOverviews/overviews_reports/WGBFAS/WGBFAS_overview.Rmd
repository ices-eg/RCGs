...
More information on data usage and publication rights can be found under the 
[ICES Data policy](https://www.ices.dk/data/guidelines-and-policy/pages/ices-data-policy.aspx "ICES Data Policy")
...
#setwd("C:/Dateien/Workshops und Fortbildungen/RCG Baltic/RCG Github/RCGs/RegionalOverviews")
...
figures <- "yes" #yes / no - all figure output to a folder
tables <- "yes" #yes / no - all table output to a folder
...
subchunkify <- function(g, chunk_name = chunk_name, fig_height=7, fig_width=5) {
  ...
  sub_chunk <- paste0("`","``{r fig_", chunk_name, ", fig.height=", fig_height, ", fig.width=", fig_width, ", echo=FALSE, result = \"asis\", warning = F, fig.pos = \"H\"}",
  "  \n (", 
    g_deparsed
    , ")()", "  \n ",
  "  \n `","``")
...
colour_table <-
     read.table(
         "C:/Dateien/Workshops und Fortbildungen/RCG Baltic/RCG Github/RCGs/RegionalOverviews/data/aux_colours.txt",
         header = T,
         sep = "\t",
...
{
  load(
    paste(data_dir, "RDB_RCG_BA_CL_2009_2022_prepared_20230414.Rdata", sep = "")
  )
  load(
    paste(data_dir, "RDB_RCG_BA_CE_2009_2022_prepared_20230414.Rdata", sep = "")
  )
  load(
    paste(data_dir, "RDB_RCG_BA_CS_all_2022_2022_prepared_202304141601.Rdata", sep = "")
  )
}
...
bar_ce <- barplot_var_by_one_var(
          ...
          subchunkify(bar_ce[[2]],"Figure_effort_ft_by_metier_lvl5",  5, 8)
          
          if (tables == "yes") {
            write.table(
              bar_ce[[1]],
              file =  
                paste(table_dir, "Figure_effort_ft_by_metier_lvl5",
                ".txt",
                sep = ""
              ),
              sep = "\t",
...
```{r stocks, results = "asis", eval = T}

cl%>%filter(SpeciesAphiaID==species_code)%>%
  distinct(stockNew)->stocks_n

for( i  in 1:nrow(stocks_n)){
cat("  \n")
cat(paste0("## ", "Annual landings by Statistical Rectangle - ",stocks_n[i]," \n"))
res <- choroplethMap_func(
            df = cl%>%filter(stockNew %in% (stocks_n[i])),
            var = "LandingWeight_1000ton",
            groupBy = "StatisticalRectangle",
            facet = "Year",
            func = "sum",
          ...
          subchunkify(res[[2]],paste0("Figure_landings_stock_",i),  10, 9)
          
          if (tables == "yes") {
            write.table(
              res[[1]],
              file =  
                paste(table_dir, paste0("Figure_landings_stock_",i),
                ".txt",
                sep = ""
              ),
              sep = "\t",
          ...
          cat("\n")
          cat(paste0("**Figure ",16+i,".**"), res[[3]],"  \n ")
          
cat('  \n')

 cat(paste0("## ", "Metier lvl5- ",stocks_n[i], " \n"))

 bar_cl <- barplot_var_by_one_var(
             x = as.data.frame(cl%>%filter(stockNew %in% (stocks_n[i]))),
             Var = 'LandingWeight_1000ton' ,
             var1 = 'FishingActivityLvl5',
             tapply_type = 'sum',
             type_of_threshold = 'top_n',
             value_of_threshold = 3,
             sorted = T,
             graph_par = eval(parse(text = 'list(oma = c(6,1,1,1), mai = c(1,1,.5,.5), ylab_line=4, col="colour5", mfrow=c(2,1), cex.x=0.7)')),
             filter = ''
           )

           subchunkify(bar_cl[[2]],paste0('Figure_landings_by_metier_lvl5_',i),  5, 8)

           if (tables == "yes") {
             write.table(
               bar_cl[[1]],
               file =
                 paste(table_dir, paste0('Figure_landings_by_metier_lvl5_',i),
                 ".txt",
                 sep = ""
               ),
               sep = '\t',
 ...
 cat('\n')
 cat(paste0("**Figure ",17+i,".**"),' Landings (in kt) by Metier Level 5. \n ')
 cat('  \n')
}
