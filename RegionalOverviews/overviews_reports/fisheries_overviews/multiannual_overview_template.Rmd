```{r title, include = F}
RCG <- "RCG_NSEA" # RCG_NA, RCG_BA or RCG_NSEA
...
---
title: `r front_title`
author: "RCG ISSG on RDB catch, effort and sampling overviews"
...
year <- 2020 # 2021 OK for fleet register
...
data_dir <- paste("C:/Users/hoai0001/Documents/Arbete/RCG_ISSG/", RCG, "/", sep = '') # Hongru's path
...
# 		sum(is.na(fleetreg$Ton_Gt))
    ...
    mapply(assign, 
       c("BEL", "DEU", "DNK", "ENG", "ESP", "EST", "FRA", "GBR", "IRL", "LTU", "LVA", "NIR", "NLD", "POL", "PRT", "SCT", "SWE", "WLS",
       "FRO", "GRL", "ISL", "ITA", "NOR", "RUS", "CHA", "CAN"), 
       c("#E6696A", "#9A9600", "#779F00", "#3EA600", "#00AC3D", "#00B06D", "#00B0B1", "#00A3E2", "#D362E3",
                 "#fffac8", "#800000", "#aaffc3", "#808000", "#a9a9a9", "#ffffff", "#000000", "#00FF00", "#FF0000",
                 "#E7298A", "#0096F0", "#9A6324", "#ffd8b1", "#000075", "#ffd8b1", "#3cb44b", "#DA7635"), 
       MoreArgs = list(envir = .GlobalEnv))

```
**Disclaimer**

The tables, plots and graphs presented in this document and the overviews are made for the coordination of regional fisheries data collection purposes and are not designed for any other use. Data used for producing the outputs are extracted from the Regional Database (RDB) and EU Fleet Register. Dates of data extractions are stated in Appendix A. Due to different aggregations and reporting authorities, data can differ to those e.g. used for assessments or technical reports.
Member States (MS) are responsible for uploading latest data and the latest year should be viewed as provisional. Data can be resubmitted by a MS for more than one previous year so there might be differences in earlier year reports if countries update back in time. Responsibility for the quality of the data and comparability to other data sources lies with the MS that provided these data. The upload logs regarding the RDB data submission, presented by the MS, can be found in the ISSG Data Quality report.
...
The annual fishing effort in number of days at sea for the `r title_stock`, is presented in figures below. Since this is not a mandatory field in the reported CE data, there is some missing information from some countries (see Table 4).  The first panel ('Days at sea Total fisheries') gives a general idea on the annual total fishing effort by country. The following panels present the annual fishing effort distribution among the different fisheries by country, where the "Fishing activity level 5" data is grouped according to métier level 3 definition: the "Pelagic trawl fisheries" include the mid-water otter trawl (OTM) and pelagic pair trawl (PTM); the "Surrounding nets fisheries" include the lampara nets (LA) and purse seine (PS); the "Seine fisheries" include the beach and boat seines (SB and SV), the fly shooting seine (SSC), anchored seine (SDN) and pair seine (SPR); the "Net fisheries" include set gillnet (GNS), driftnet (GND) and trammel net (GTR); the "Longline fisheries" include set and drifting longlines (LLS and LLD); the "Rods and lines fisheries" include the hand and pole lines (LHP and LHM) and the trolling lines (LTL); the "Dredge fisheries" include boat and hand dredges (DRB and HMD); and "Traps fisheries" include the pots and traps (FPO), the stationary uncovered pound nets (FPN) and fyke nets (FYK). In the case of the bottom trawls, the panels are presented by metier level 4 because of the higher number of metiers level 5 present in this group: "Bottom otter trawl fisheries" (OTB); "Multi-rig otter trawl fisheries" (OTT); "Bottom pair trawl fisheries" (PTB); "Beam trawl fisheries" (TBB). Note that the presence of "blank plots" is related to single year data in a particular gear group.

Table 4. Summary of the completeness of the information on days at sea reported by country
##### - to include Laura table
...
issgPalette <- c(FRO, GRL, ISL, ITA, NOR, RUS, CHA, CAN, BEL, DEU, DNK, ENG, ESP, EST, FRA, GBR, IRL, LTU, 
                LVA, NIR, NLD, POL, PRT, SCT, SWE, WLS)
#issgPalette <- c(BEL, CHA, DEU, DNK, ENG, ESP, EST, FRA, GBR, IRL, LTU, LVA, NIR, NLD, POL, PRT, SCT, SWE, WLS)

dasTotal <- dasTotal + scale_color_manual(values=issgPalette)
...
#issgPalette <- c("gold4", "black", "#C60C30", "tomato1", "#F1BF00", "#0072CE", "#002395", "#012169",
#                 "#046A38", "#77353D", "#031330", "#FF9B00", "pink", "#046A38", "deepskyblue1", "#FFCD00", "#82425d")
dasCountryOTB <- dasCountryOTB + scale_color_manual(values=issgPalette)
...
dasCountryOTT <- ggplot(data=DAS_country_OTT, aes(Year, DaysAtSea, colour=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Days at sea by country Multi-rig otter trawl fisheries -", RCG), x="year", y="Days at sea")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 
  
dasCountryOTT <- dasCountryOTT + scale_color_manual(values=issgPalette)
...
dasCountryPTB <- ggplot(data=DAS_country_PTB, aes(Year, DaysAtSea, colour=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Days at sea by country Bottom pair trawl fisheries -", RCG), x="year", y="Days at sea")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 

dasCountryPTB <- dasCountryPTB + scale_color_manual(values=issgPalette)
...
dasCountryTBB <- ggplot(data=DAS_country_TBB, aes(Year, DaysAtSea, colour=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Days at sea by country Beam trawl fisheries -", RCG), x="year", y="Days at sea")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 

dasCountryTBB <- dasCountryTBB + scale_color_manual(values=issgPalette)
...
dasCountryOTM <- dasCountryOTM + scale_color_manual(values=issgPalette)
...
dasSNet <- ggplot(data=DAS_country_SNet, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasSNet <- dasSNet + scale_color_manual(values=issgPalette)
...
dasS <- ggplot(data=DAS_country_S, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasS <- dasS + scale_color_manual(values=issgPalette)
...
dasN <- ggplot(data=DAS_country_N, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasN <- dasN + scale_color_manual(values=issgPalette)
...
dasLL <- ggplot(data=DAS_country_LL, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasLL <- dasLL + scale_color_manual(values=issgPalette)
...
dasRL <- ggplot(data=DAS_country_RL, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasRL <- dasRL + scale_color_manual(values=issgPalette)
...
dasD <- ggplot(data=DAS_country_D, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasD <- dasD + scale_color_manual(values=issgPalette)
...
dasT <- ggplot(data=DAS_country_T, aes(Year, DaysAtSea, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
dasT <- dasT + scale_color_manual(values=issgPalette)
...
The annual landings overview for the `r title_stock`, is presented in figures below. The first panel ('Landings Total fisheries') compiles the annual total landings by country. The following panels present the annual total landings from the different fisheries by country, where the "Fishing activity level 5" data is grouped according to métier level 3 definition: the "Pelagic trawl fisheries" include the mid-water otter trawl (OTM) and pelagic pair trawl (PTM); the "Surrounding nets fisheries" include the lampara nets (LA) and purse seine (PS); the "Seine fisheries" include the beach and boat seines (SB and SV), the fly shooting seine (SSC), anchored seine (SDN) and pair seine (SPR); the "Net fisheries" include set gillnet (GNS), driftnet (GND) and trammel net (GTR); the "Longline fisheries" include set and drifting longlines (LLS and LLD); the "Rods and lines fisheries" include the hand and pole lines (LHP and LHM) and the trolling lines (LTL); the "Dredge fisheries" include boat and hand dredges (DRB and HMD); and "Traps fisheries" include the pots and traps (FPO), the stationary uncovered pound nets (FPN) and fyke nets (FYK). In the case of the bottom trawls, the panels are presented by metier level 4 because of the higher number of metiers level 5 present in this group: "Bottom otter trawl fisheries" (OTB); "Multi-rig otter trawl fisheries" (OTT); "Bottom pair trawl fisheries" (PTB); "Beam trawl fisheries" (TBB). Note that the presence of "blank plots" is related to single year data in a particular gear group. Note that the presence of "blank plots" is related to single year data in a particular gear group.
...
landingTotal <- ggplot(data=Landings_total, aes(Year, round(LandingWeight_ton/1000,0), colour=FlagCountry)) +
  geom_point() + geom_line() +
  scale_colour_discrete(guide = 'none') +
  geom_dl(aes(label = FlagCountry), method = list(dl.trans(x = x + 0.1), "last.points", cex = 0.7))+
...
landingTotal <- landingTotal + scale_color_manual(values=issgPalette)

ggplotly(landingTotal)
```
...
landingOTB <- ggplot(data=Landings_country_OTB, aes(Year, LandingWeight_ton, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Landings by country Bottom trawl fisheries -", RCG), x="year", y="Landings (ton)")+
  theme(legend.position="bottom",axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90))

landingOTB <- landingOTB + scale_color_manual(values=issgPalette)
...
landingsOTT <- ggplot(data=Landings_country_OTT, aes(Year, LandingWeight_ton, colour=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Landings by country Multi-rig otter trawl fisheries -", RCG), x="year", y="Landings (ton)")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 

landingsOTT <- landingsOTT + scale_color_manual(values=issgPalette)
...
```{r echo=FALSE, message=FALSE, warning = FALSE, out.width="90%", out.height="90%"}
Landings_country_PTB <- droplevels(Landings_country[!is.na(Landings_country$LandingWeight_ton) & DAS_country$gear %in% c('PTB'),])

landingsPTB <- ggplot(data=Landings_country_PTB, aes(Year, LandingWeight_ton, colour=FlagCountry)) +
  geom_point() + geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Landings by country Bottom pair trawl fisheries -", RCG), x="year", y="Landings (ton)")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 

landingsPTB <- landingsPTB + scale_color_manual(values=issgPalette)
...
landingsTBB <- ggplot(data=Landings_country_TBB, aes(Year, LandingWeight_ton, colour=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
  labs(title=paste("Landings by country Beam trawl fisheries -", RCG), x="year", y="Landings (ton)")+
  theme(legend.position="bottom", axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90)) 

landingsTBB <- landingsTBB + scale_color_manual(values=issgPalette)
...
landingsOTM <- landingsOTM + scale_color_manual(values=issgPalette)
...
landingsSNet <- landingsSNet + scale_color_manual(values=issgPalette)
...
landingsS <- landingsS + scale_color_manual(values=issgPalette)
...
landingsN <- landingsN + scale_color_manual(values=issgPalette)
...
landingsLL <- landingsLL + scale_color_manual(values=issgPalette)
...
landingsRL <- landingsRL + scale_color_manual(values=issgPalette)
...
landingsD <- landingsD + scale_color_manual(values=issgPalette)
...
landingsT <- ggplot(data=Landings_country_T, aes(Year, LandingWeight_ton, colour=FlagCountry)) +
  geom_point() +geom_line() +
  theme_bw() +
  facet_wrap(~FishingActivityLvl5, scales="free_y")+
...
landingsT <- landingsT + scale_color_manual(values=issgPalette)
...
land1 <- ggplot(data=landings_sp_sum, aes(Year, round(Landing_ton/1000,0), colour=topSpecies)) +
  geom_point() + geom_line() +
  labs(title=paste("Landings by top 10 species -", RCG), x="year", y="Landings (thousand ton)")+
  theme(axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8))+
  theme_bw()
...
land3 <- ggplot(data=landings_sp_sum, aes(x=Year, y=Landing_ton, fill=topSpecies)) +
  ...
  labs(title=paste("Landings by top 10 species -", RCG), x="year", y="Proportion of landings")+
  theme(axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8))+
  theme_bw()
ggplotly(land3)
```

### 4 Landings by country top 10 species - stacked bar pct
```{r echo=FALSE, message=FALSE, warning = FALSE, out.width="90%", out.height="90%"}
CL_years$Species1 <- as.character(CL_years$Species)
top10_species <- CL_years %>%
  group_by(Species1) %>%
...
Warning: This stock allocation represents variable stock denominations which were adjusted by the group accordingly with procedures discussed within ICES community and therefore it does not come directly from the RDB. `r warning_stock`

### Landings by country top 5 stocks
(text needing revision)
The table below summarizes the Top 5 stocks in Landing’s percentage incidence per country, and by year, from `r years[1]` to `r years[5]`. This stock information has been developed for the last five years scope where adjustments and definitions are being worked ever since. In the table, landings per stock ("Landings") and total landings of all stocks by country and year ("Total") are presented in thousand tons. (However, regarding this first picture, main stocks had been roughly the same and the Top 5 range only floated slightly.) 
...
# Check if Nuno changed this to evaluate the need to ADD INFORMATION ABOUT THIS IN THE TEXT
...
This set of graphs refer to the Landing’s percentage incidence of the top 10 stocks per country. The main goal of this topic is to present how those stock landings are distributed among countries and analyse the evolution of the landings by stock in the reference period (`r years[1]` - `r years[5]`). It also gives some indications on the stock incidence history per country.
...
ggplot(data=landings_stock_sum, aes(x=Year, y=round(Landing_s/1000,0), fill=FlagCountry)) +
  ...
  theme_bw() +
  facet_wrap(~topstock)+
  labs(title=paste("Landings by top 10 stocks -", RCG), x="year", y="Proportion of landings")+
  theme(axis.text.x = element_text(size=8), plot.title=element_text(size=8), axis.title = element_text(size=8),
        axis.text.x.bottom  = element_text(angle = 90))
