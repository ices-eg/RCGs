...
```{r title, include = F}
RCG <- "BA" # RCG_NA, RCG_BA or RCG_NSEA
...
The most landed species in weight, in `r title_stock` are presented in the plots from Figure 3. The first three panels show the top 10 species present in the landings of the region, using different layouts (1:lines; 2:stacked bars; 3: stacked bar proportions). The last panel presents the countries that contribute for the landings of the main landed species in weight (top 10) in the `r title_stock`.

## Figures 3: Landings by Species {.tabset}
...
ggplot(data=landings_sp_sum, aes(Year, Landing_ton/1000, colour=topSpecies)) +
  geom_point() + geom_line() +
...
ggplot(data=landings_sp_sum, aes(x=Year, y=Landing_ton/1000, fill=topSpecies)) +
...
## Stock {.tabset}
The following chapter refers to the multiannual stocks’ distribution for the `r title_stock`. This data is provided from ICES’ fishframe RDB regional database that has been working with the development and production of fisheries data.
...
temp$new_stock<-ifelse(is.na(temp$Code),temp$stock,temp$Code)
