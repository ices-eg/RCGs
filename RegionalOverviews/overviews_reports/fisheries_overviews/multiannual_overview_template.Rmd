...
if (RCG == "NA") {
  front_title <- paste("RDB Catch and Effort Overview North Atlantic ",  "  \n ","![](../../overviews_shiny/www/RCGs_logo2.png)", sep = "")
} else if (RCG == "BA") {
  front_title <- paste("RDB Catch and Effort Overview Baltic ", "  \n ","![](../../overviews_shiny/www/RCGs_logo2.png)", sep = "")
} else if (RCG == "NSEA") {
  front_title <- paste("RDB Catch and Effort Overview North Sea and Eastern Arctic ", "  \n ","![](../../overviews_shiny/www/RCGs_logo2.png)", sep = "")
}
...
## Stock {.tabset}
### Year|Country|Stock|FishingGround|Species|Landings|Total Landings|Percent
```{r echo=FALSE, message=FALSE, warning = FALSE, out.width="90%", out.height="90%"}
...
temp<-left_join(CL_years,Def_stock_KK, by='ID', all.x=TRUE)
temp$new_stock<-ifelse(is.na(temp$Code),temp$stock,temp$Code)
#if still new_stock is na ->catch_group
# temp$new_stockplus<-ifelse(is.na(temp$new_stock),paste(temp$catch_group,temp$Area,sep=''),temp$new_stock)
