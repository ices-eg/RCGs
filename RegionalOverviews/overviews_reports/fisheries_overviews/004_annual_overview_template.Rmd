...
{
  load(
    paste(data_dir, "RDB_RCG_BA_CL_2009_2018_prepared_201905101612.Rdata", sep = "")
  )
  load(
    paste(data_dir, "RDB_RCG_BA_CE_2009_2018_prepared_201905101612.Rdata", sep = "")
  )
}
...
```{r, results = 'asis'}
  ...
  year_pyear<-as.data.frame.matrix(rbind((res_fleetreg[,,1]-res_fleetreg[,,2]),apply((res_fleetreg[,,1]-res_fleetreg[,,2]),2,sum)))
...
```{r, results = 'asis'}
  ...
  year_pyear<-as.data.frame.matrix(round(rbind((res_fleetreg_pwr[,,1]-res_fleetreg_pwr[,,2]),apply((res_fleetreg_pwr[,,1]-res_fleetreg_pwr[,,2]),2,sum))/1000,1))
ft<-fun_table(year,year_pyear)
ft
...
```{r, results = 'asis'}
  ...
  year_pyear<-as.data.frame.matrix(round(rbind((res_fleetreg_gt[,,1]-res_fleetreg_gt[,,2]),apply((res_fleetreg_gt[,,1]-res_fleetreg_gt[,,2]),2,sum))/1000,1))
ft<-fun_table(year,year_pyear)
ft
```
