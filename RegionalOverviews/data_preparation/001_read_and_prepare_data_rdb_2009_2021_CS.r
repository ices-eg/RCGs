# RCG subgroup work on Regional Fisheries and Sampling Overview
	...
	# Ana Cláudia: script updated for 2020 and 2021 data
	# 20.04.2022: included field for the sampling type description (to be used by the shiny app)
...
# note on CS subsets
 ...
 gc()
	...
	## sets directory
# ACF portatil
#setwd('C:\\Users\\acfernandes\\Documents\\2022\\000_RCG_Interssessional Work\\ISSG Catch Effort Sampling Overviews_old\\ISSG CESOv\\RegionalOverviews\\New_data\\')
...
# ========================
 ...
 # read file names
	...
	setwd('..\\RegionalOverviews\\New_data\\') 
  
 	file_tr <- "CS TR Trip.csv" #"data/001_original/2020/20200417/TR Trip.csv"
	file_hh <- "CS HH Station.csv" #"data/001_original/2020/20200417/HH Station.csv" 
	file_sl <- "CS SL SpeciesList.csv"	#"data/001_original/2020/20200417/SL SpeciesList.csv" 
	file_hl <- "CS HL Length.csv"	#"data/001_original/2020/20200417/HL Length.csv"  
	file_ca <- "CS CA SMAWL.csv"	#"data/001_original/2020/20200417/CA SMAWL.csv" 
...
dir.create(paste("002_prepared/2021/RCG_NA", sep=""),recursive=TRUE, showWarnings=FALSE)
dir.create(paste("002_prepared/2021/RCG_BA", sep=""),recursive=TRUE, showWarnings=FALSE)
dir.create(paste("002_prepared/2021/RCG_NSEA", sep=""),recursive=TRUE, showWarnings=FALSE) 
...
year_start <- 2009
year_end <- 2021
dir_output_rcg<-paste("002_prepared/2021/",target_region,sep="")
dir_output_all<-"002_prepared/2021/"
...
# ========================	
	## Change the names of the sampling Type (D = MarketSamMixTrips, S = AtSeaSamTrips, M = MarketSamTrips, V = Vendor) - it's more easy here than in the shiny app (ACF, 19.04.2022)
	# EXAMPLE
	# setnames(ca_rcg, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
	# ca_rcg[, SamplingType := NA] # creates new column with NA
	# # Assigns description of the sampling type codes in the new column
	# ca_rcg[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
	# ca_rcg[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
	# ca_rcg[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
	# ca_rcg[SamplingType_orig=="V", SamplingType := "Vendor"]
	...
	setnames(tr, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		tr[, SamplingType := NA] # creates new column with NA
		tr[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		tr[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		tr[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		tr[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		tr[SamplingType_orig=="V", SamplingType := "Vendor"]
	setnames(tr_rcg_all, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		tr_rcg_all[, SamplingType := NA] # creates new column with NA
		tr_rcg_all[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		tr_rcg_all[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		tr_rcg_all[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		tr_rcg_all[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		tr_rcg_all[SamplingType_orig=="V", SamplingType := "Vendor"]
	
		# hh
	setnames(hh, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		hh[, SamplingType := NA] # creates new column with NA
		hh[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		hh[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		hh[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		hh[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		hh[SamplingType_orig=="V", SamplingType := "Vendor"]
	setnames(hh_rcg_all, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		hh_rcg_all[, SamplingType := NA] # creates new column with NA
		hh_rcg_all[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		hh_rcg_all[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		hh_rcg_all[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		hh_rcg_all[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		hh_rcg_all[SamplingType_orig=="V", SamplingType := "Vendor"]	
		
		# sl
		sl[, Weight := as.numeric(Weight)]	## added (it was as 'integer64')
		sl[, Weight_kg := Weight/1000]
		sl[, Weight_ton := Weight/1000000]
		sl[, SubSampleWeight := as.numeric(SubSampleWeight)]	## added (it was as 'integer64')
		sl[, SubSampleWeight_kg := SubSampleWeight/1000]
		sl[, SubSampleWeight_ton := SubSampleWeight/1000000]
		sl_rcg_all[, Weight:= as.numeric(Weight)]	## added (it was as 'integer64')
		sl_rcg_all[, Weight_kg := Weight/1000]
		sl_rcg_all[, Weight_ton := Weight/1000000]
		sl_rcg_all[, SubSampleWeight := as.numeric(SubSampleWeight)]	## added (it was as 'integer64')
		sl_rcg_all[, SubSampleWeight_kg := SubSampleWeight/1000]
		sl_rcg_all[, SubSampleWeight_ton := SubSampleWeight/1000000]

	setnames(sl, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		sl[, SamplingType := NA] # creates new column with NA
		sl[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		sl[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		sl[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		sl[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		sl[SamplingType_orig=="V", SamplingType := "Vendor"]
	setnames(sl_rcg_all, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		sl_rcg_all[, SamplingType := NA] # creates new column with NA
		sl_rcg_all[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		sl_rcg_all[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		sl_rcg_all[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		sl_rcg_all[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		sl_rcg_all[SamplingType_orig=="V", SamplingType := "Vendor"]
	...
	setnames(hl, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		hl[, SamplingType := NA] # creates new column with NA
		hl[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		hl[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		hl[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		hl[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		hl[SamplingType_orig=="V", SamplingType := "Vendor"]
	setnames(hl_rcg_all, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		hl_rcg_all[, SamplingType := NA] # creates new column with NA
		hl_rcg_all[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		hl_rcg_all[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		hl_rcg_all[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		hl_rcg_all[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		hl_rcg_all[SamplingType_orig=="V", SamplingType := "Vendor"]
		...
		setnames(ca, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		ca[, SamplingType := NA] # creates new column with NA
		ca[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		ca[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		ca[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		ca[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		ca[SamplingType_orig=="V", SamplingType := "Vendor"]
	setnames(ca_rcg_all, 'SamplingType', 'SamplingType_orig')# renames SampingType (V, M, S, D) as the original data
		ca_rcg_all[, SamplingType := NA] # creates new column with NA
		ca_rcg_all[, SamplingType := as.character(SamplingType)]
	 # Assigns description of the sampling type codes in the new column
		ca_rcg_all[SamplingType_orig=="D", SamplingType := "MarketSamMixTrips"]
		ca_rcg_all[SamplingType_orig=="S", SamplingType := "AtSeaSamTrips"]
		ca_rcg_all[SamplingType_orig=="M", SamplingType := "MarketSamTrips"]
		ca_rcg_all[SamplingType_orig=="V", SamplingType := "Vendor"]
	...
	# QCA: visual
		hh[, list(N=.N),list(AreaMap,Area)][order(AreaMap)]
		hh_rcg_all[, list(N=.N),list(AreaMap,Area, FlagCountry, Year)][order(AreaMap)][AreaMap=="NA",]
		ca[, list(N=.N),list(AreaMap,Area)][order(AreaMap)]
...
# ========================			
		...
		hh[StatisticalRectangle=="99u9", StatisticalRectangle := NA]
		ca[StatisticalRectangle=="99u9", StatisticalRectangle := NA]
		
		hh[StatisticalRectangle=="99x9", StatisticalRectangle := NA]
		ca[StatisticalRectangle=="99x9", StatisticalRectangle := NA]

		hh_rcg_all[StatisticalRectangle=="99u9", StatisticalRectangle := NA]
		ca_rcg_all[StatisticalRectangle=="99u9", StatisticalRectangle := NA]
		
		hh_rcg_all[StatisticalRectangle=="99x9", StatisticalRectangle := NA]
		ca_rcg_all[StatisticalRectangle=="99x9", StatisticalRectangle := NA]
...
# ========================		
	...
	#ASFIS_WoRMS_updt <- read.table (file="../RegionalOverviews/data/ASFIS_WoRMS_updt.csv", header=T, sep=";", stringsAsFactors=FALSE) # on the data sharepoint of the RCG subgroup
	#ASFIS_WoRMS_updt <- read.table (file="C:/Users/acfernandes/Documents/2022/000_RCG_Interssessional Work/ISSG Catch Effort Sampling Overviews_old/ISSG CESOv/RegionalOverviews/data/ASFIS_WoRMS_updt.csv", header=T, sep=";", stringsAsFactors=FALSE)
	ASFIS_WoRMS_updt <- read.table (file="D:/Documents/PNAB/2022/000_RCG_InterssessionalWork/ISSG_Catch_Effort_Sampling_overviews/ISSG_overviews/RegionalOverviews/data/ASFIS_WoRMS_updt.csv", header=T, sep=";", stringsAsFactors=FALSE)
		...
		# =====================	
			...
			sl_rcg_all[Species == "Alepocephalus agassizii",ISSCAAP:=34] # Miscellaneous demersal fishes
			sl_rcg_all[Species == "Cottus gobio",ISSCAAP:=13] # Miscellaneous freshwater fishes
			sl_rcg_all[Species == "Gobius niger",ISSCAAP:=33] # Miscellaneous coastal fishes
			sl_rcg_all[Species == "Gymnocephalus cernua",ISSCAAP:=34] # Miscellaneous demersal fishes
			sl_rcg_all[Species == "Liparis liparis",ISSCAAP:=34] # Miscellaneous demersal fishes
			sl_rcg_all[Species == "Lumpenus lampretaeformis",ISSCAAP:=33] # Miscellaneous coastal fishes
			...
			sl_rcg_all[grepl(Species, pat = "Gasterosteidae"),ISSCAAP:=25] # Miscellaneous diadromous fishes
			...
			sl_rcg_all[grepl(Species, pat = "Inachus"),ISSCAAP:=42]  # Crabs, sea-spiders
				...
				# and the few remaining (manually...)
					...
					sl_rcg_all[Species == "Ascidiella scabra",ISSCAAP:=76] # Sea-urchins and other echinoderms
		...
		# Adds RCG Catch_group
			#aux_spp_categ<-read.table("../RegionalOverviews/data/Table_Species_Categ.txt", header=T, sep="\t")
			#aux_spp_categ<-read.table("C:/Users/acfernandes/Documents/2022/000_RCG_Interssessional Work/ISSG Catch Effort Sampling Overviews_old/ISSG CESOv/RegionalOverviews/data/Table_Species_Categ.txt", header=T, sep="\t")
			aux_spp_categ <- read.table (file="D:/Documents/PNAB/2022/000_RCG_InterssessionalWork/ISSG_Catch_Effort_Sampling_overviews/ISSG_overviews/RegionalOverviews/data/Table_Species_Categ.txt", header=T, sep="\t")
		...
		# adds ISCAAP and Catch_group to ca (based on sl and )
			...
			ca_rcg_all[Species == "Alepocephalus agassizii",ISSCAAP:=34] # Miscellaneous demersal fishes
			ca_rcg_all[Species == "Alepocephalus agassizii",Catch_group:="demersal"]
...
# =====================
	...
	hh$StartDate <- as.character(hh$StartDate)	
	hh_rcg_all$StartDate <- as.character(hh_rcg_all$StartDate)
	hh_rcg$StartDate <- as.character(hh_rcg$StartDate)
...
########################################################
	...
	tr_rcg_all <- droplevels(tr_rcg_all[tr_rcg_all$Year==2021,])
	tr_rcg <- droplevels(tr_rcg[tr_rcg$Year==2021,])
	tr <- droplevels(tr[tr$Year==2021,])
	hh_rcg_all <- droplevels(hh_rcg_all[hh_rcg_all$Year==2021,])
	hh_rcg <- droplevels(hh_rcg[hh_rcg$Year==2021,])
	hh <- droplevels(hh[hh$Year==2021,])
	sl_rcg_all <- droplevels(sl_rcg_all[sl_rcg_all$Year==2021,])
	sl_rcg <- droplevels(sl_rcg[sl_rcg$Year==2021,])
	sl <- droplevels(sl[sl$Year==2021,])
	hl_rcg_all <- droplevels(hl_rcg_all[hl_rcg_all$Year==2021,])
	hl_rcg <- droplevels(hl_rcg[hl_rcg$Year==2021,])
	hl <- droplevels(hl[hl$Year==2021,])
	ca_rcg_all <- droplevels(ca_rcg_all[ca_rcg_all$Year==2021,])
	ca_rcg <- droplevels(ca_rcg[ca_rcg$Year==2021,])
	ca <- droplevels(ca[ca$Year==2021,])
		
# ========================
# saves data for 2021
# ========================		
	
	save(tr_rcg_all, hh_rcg_all, sl_rcg_all, hl_rcg_all ,ca_rcg_all, file_info_cs, file = paste(dir_output_rcg, paste("\\RDB",target_region,"CS_all", "2021", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))
	save(tr_rcg, hh_rcg, sl_rcg, hl_rcg ,ca_rcg, file_info_cs, file = paste(dir_output_rcg, paste("\\RDB",target_region,"CS_rcg", "2021", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))
	save(tr, hh, sl, hl, ca, file_info_cs, file = paste(dir_output_all, paste("\\RDB","All_Regions","CS", "2021", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))	
