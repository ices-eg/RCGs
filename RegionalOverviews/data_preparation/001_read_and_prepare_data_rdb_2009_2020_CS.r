...
year_start <- 2009
year_end <- 2020
dir_output_rcg<-paste("002_prepared/2020/",target_region,sep="")
dir_output_all<-"002_prepared/2020/"
...
########################################################
	...
	tr_rcg_all <- droplevels(tr_rcg_all[tr_rcg_all$Year==2020,])
	tr_rcg <- droplevels(tr_rcg[tr_rcg$Year==2020,])
	tr <- droplevels(tr[tr$Year==2020,])
	hh_rcg_all <- droplevels(hh_rcg_all[hh_rcg_all$Year==2020,])
	hh_rcg <- droplevels(hh_rcg[hh_rcg$Year==2020,])
	hh <- droplevels(hh[hh$Year==2020,])
	sl_rcg_all <- droplevels(sl_rcg_all[sl_rcg_all$Year==2020,])
	sl_rcg <- droplevels(sl_rcg[sl_rcg$Year==2020,])
	sl <- droplevels(sl[sl$Year==2020,])
	hl_rcg_all <- droplevels(hl_rcg_all[hl_rcg_all$Year==2020,])
	hl_rcg <- droplevels(hl_rcg[hl_rcg$Year==2020,])
	hl <- droplevels(hl[hl$Year==2020,])
	ca_rcg_all <- droplevels(ca_rcg_all[ca_rcg_all$Year==2020,])
	ca_rcg <- droplevels(ca_rcg[ca_rcg$Year==2020,])
	ca <- droplevels(ca[ca$Year==2020,])
		
# ========================
# saves data for 2020
# ========================		
	
	save(tr_rcg_all, hh_rcg_all, sl_rcg_all, hl_rcg_all ,ca_rcg_all, file_info_cs, file = paste(dir_output_rcg, paste("\\RDB",target_region,"CS_all", "2020", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))
	save(tr_rcg, hh_rcg, sl_rcg, hl_rcg ,ca_rcg, file_info_cs, file = paste(dir_output_rcg, paste("\\RDB",target_region,"CS_rcg", "2020", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))
	save(tr, hh, sl, hl, ca, file_info_cs, file = paste(dir_output_all, paste("\\RDB","All_Regions","CS", "2020", year_end, "prepared",time_tag, sep="_"),".Rdata", sep=""))	
