...
# read file names
	...
	file_cl <- "CL landing.csv"
	file_ce <- "CE effort.csv" 
...
dir.create(paste("002_prepared/2020/RCG_NA", sep=""),recursive=TRUE, showWarnings=FALSE)
dir.create(paste("002_prepared/2020/RCG_BA", sep=""),recursive=TRUE, showWarnings=FALSE)
dir.create(paste("002_prepared/2020/RCG_NSEA", sep=""),recursive=TRUE, showWarnings=FALSE)
...
dir_output_rcg<-paste("002_prepared/2020/",time_tag,"/",target_region,sep=""); dir.create(dir_output_rcg, recursive=T)
dir_output_all<-paste("002_prepared/2020/",time_tag, sep="/"); dir.create(dir_output_all, recursive=T)
...
# ========================	
	...
	# CL
		...
		# 'BEN' (Benim - 'BJ' according to UNLOCODE lists) and 'ESH' (Western Sahara - 'EH' according to UNLOCODE lists) are not included in the file 'aux_countries' - changed with code lines:
			cl[HarbourCountry2 %in% c("EH"), HarbourCountry := "EH"]
			cl_rcg[HarbourCountry2 %in% c("EH"), HarbourCountry := "EH"]
			
			cl[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BJ"]
			cl_rcg[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BJ"]
	...
	# CE 
		...
		# HarbourCountry (ISO3) and HarbourCountry2 (ISO2)			
			...
			# 'BEN' (Benim - 'BJ' according to UNLOCODE lists) and 'ESH' (Western Sahara - 'EH' according to UNLOCODE lists) are not included in the file 'aux_countries' - changed with code lines:
			ce[HarbourCountry2 %in% c("EH"), HarbourCountry := "EH"]
			ce_rcg[HarbourCountry2 %in% c("EH"), HarbourCountry := "EH"]
			
			ce[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BJ"]
			ce_rcg[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BJ"]
...
# ================	
	...
	# all countries: 2018-2020
		cl[Year %in% c(2018,2019,2020),sum(OfficialLandingCatchWeight),list(FlagCountry, Year)] [order(FlagCountry, Year)]
		ce[Year %in% c(2018,2019,2020),sum(TripsNumber),list(FlagCountry, Year)] [order(FlagCountry, Year)]
...
# reads aux_countries dataset
	...
	cl[!is.na(stockNew) & Year==2020,list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, stockNew)][order(-V1),]
	cl_rcg[!is.na(stockNew) & Year==2020,list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, stockNew)][order(-V1),]
	
	cl[!is.na(stockNew) & Year==2020,list(V1=sum(OfficialLandingCatchWeight)), list(stockNew)][order(-V1),][grepl(stockNew, pat="cod.27.1-2"),]
	cl_rcg[!is.na(stockNew) & Year==2020,list(V1=sum(OfficialLandingCatchWeight)), list(stockNew)][order(-V1),][grepl(stockNew, pat="cod.27.1-2"),]
	
	cl[grepl(Area, pat="27.") & Year==2020,list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)), list(Species)]
	cl_rcg[grepl(Area, pat="27.") & Year==2020,list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)), list(Species)]
	
	# species with stocks
	target_species<-unique(cl$Species[!is.na(cl$stockNew)])
		
	out<-cl[grepl(Area, pat="27.") & Year==2020 & Species %in% target_species, list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=round(sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)*100,1), areasWithoutStock=paste(sort(unique(Area[is.na(stockNew)])), collapse=","), stocksPresent=paste(sort(unique(stockNew[!is.na(stockNew)])), collapse=",")), list(Species)]
	...
	cl[grepl(Area, pat="27.") & Year==2020,,]
	cl[is.na(stockNew) & Year==2020 & FlagCountry=="ESP" & Species ==  "Gadus morhua",]
	
	head(cl[grepl(Area, pat="27.") & is.na(stockNew) & Year==2020 & FlagCountry=="SWE",list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, Area, stockNew)][order(-V1),],10)
	cl[grepl(Area, pat="27.") & is.na(stockNew) & Year==2020 & FlagCountry=="ESP",list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, Area, stockNew)][order(-V1),]
