...
file_cl <- "..\\RCG LDF Data\\2021\\CL Landing table LDF 2014 2020.csv"
file_ce <- "..\\RCG LDF Data\\2021\\CE Effort table LDF 2014 2020.csv" 
...
dir_output_all <-"..\\RCG LDF Data\\2021" 
...
dim(cl); cl<-unique(cl); dim(cl) # duplicates found, HKN was informed about it
...
# Subolygon coding
cl[,Subpolygon:=as.character(Subpolygon)]
cl[,SubpolygonCode:=Subpolygon]
cl[SubpolygonCode==15, Subpolygon := "Canaries"]  
cl[SubpolygonCode==16, Subpolygon := "Madeira"]  
cl[SubpolygonCode==17, Subpolygon := "Morocco"]  
cl[SubpolygonCode==18, Subpolygon := "Mauritania"]  
cl[SubpolygonCode==19, Subpolygon := "Senegal"]  
cl[SubpolygonCode==20, Subpolygon := "Gambia"]  
cl[SubpolygonCode==21, Subpolygon := "Guinea-Bissau"]  
cl[SubpolygonCode==22, Subpolygon := "Guinea"]  
cl[SubpolygonCode==23, Subpolygon := "Sierra Leona"]  
cl[SubpolygonCode==24, Subpolygon := "Congo"]  
cl[SubpolygonCode==25, Subpolygon := "Angola"]  

ce[,Subpolygon:=as.character(Subpolygon)]
ce[,SubpolygonCode:=Subpolygon]
ce[SubpolygonCode==15, Subpolygon := "Canaries"]  
ce[SubpolygonCode==16, Subpolygon := "Madeira"]  
ce[SubpolygonCode==17, Subpolygon := "Morocco"]  
ce[SubpolygonCode==18, Subpolygon := "Mauritania"]  
ce[SubpolygonCode==19, Subpolygon := "Senegal"]  
ce[SubpolygonCode==20, Subpolygon := "Gambia"]  
ce[SubpolygonCode==21, Subpolygon := "Guinea-Bissau"]  
ce[SubpolygonCode==22, Subpolygon := "Guinea"]  
ce[SubpolygonCode==23, Subpolygon := "Sierra Leona"]  
ce[SubpolygonCode==24, Subpolygon := "Congo"]  
ce[SubpolygonCode==25, Subpolygon := "Angola"]  
...
cl[Area %in% c("34.3.4"), Division := "34.3.4"]
...
# new: updated on the fishbase basis. it's also in the fishpi codes (Kasia)
# nowe: uzupelnione na podstawie fishbase, jest to tez w kodach z fishpi 
cl[Species == "Sepiolidae",ISSCAAP:=57]
cl[Species == "Sepiida",ISSCAAP:=57]
cl[Species == "Scorpaeniformes",ISSCAAP:=34]
cl[Species == "Seriola",ISSCAAP:=37]
cl[Species == "Plantae",ISSCAAP:=94]
cl[Species == "Latilinae",ISSCAAP:=34] # don't know how to fix this,a few records
cl[Species == "Holocephali",ISSCAAP:=38] #  don't know how to fix this,a few records
cl[Species == "Symphurus insularis",ISSCAAP:=31] 
