# RCG subgroup work on Regional Fisheries and Sampling Overview
	...
	# Ana Cláudia: script updated for 2020, 2021 data
...
# note on CS subsets
	...
	# 2022-04-20: groups data from UK countries - UK = (CHA+WLS+SCO+NIR+ENG)
	# 2022-04-20: Assigns landing country code to the missing 'harbour country' codes (due to no harbour information being available)
...
# ========================
	...
	setwd('D:\\Documents\\PNAB\\2022\\000_RCG_InterssessionalWork\\ISSG_Catch_Effort_Sampling_overviews\\ISSG_overviews\\RegionalOverviews\\New_data\\') 
...
# read file names
	...
	file_cl <- "CL Landing_20220531.csv"
	file_ce <- "CE Effort_20220531.csv" 
...
restrict_to_SSF_data <- TRUE
target_region <- "RCG_NSEA" # "RCG_NA", "RCG_BA"; RCG_NSEA
year_start <- 2009
year_end <- 2021
time_tag<-format(Sys.time(), "%Y%m%d")
#time_tag<-202006160931
dir_output_rcg<-paste("002_prepared/2021/",time_tag,"/",target_region,sep=""); dir.create(dir_output_rcg, recursive=T)
dir_output_all<-paste("002_prepared/2021/",time_tag, sep="/"); dir.create(dir_output_all, recursive=T)
...
# ========================	
	...
	# CL
		...
		# HarbourCountry (ISO3) and HarbourCountry2 (ISO2)
			...
			cl_rcg[,HarbourCountry:=aux_countries$ISO3Code[match(HarbourCountry2, aux_countries$ISO2Code)]]
				...
				## when missing harbour and considering that harbour country = landing country (excluded '*HS' and 'WK1')
				table(cl[is.na(Harbour),]$LandingCountry)
				# HarbourCountry (ISO3) and HarbourCountry2 (ISO2)
				cl[is.na(Harbour) & !LandingCountry %in% c('*HS','WK1'),HarbourCountry:=LandingCountry]
				cl_rcg[is.na(Harbour) & !LandingCountry %in% c('*HS','WK1'),HarbourCountry:=LandingCountry]
				cl[is.na(Harbour) & !LandingCountry %in% c('*HS','WK1'),HarbourCountry2:=aux_countries$ISO2Code[match(HarbourCountry, aux_countries$ISO3Code)]]
				cl_rcg[is.na(Harbour) & !LandingCountry %in% c('*HS','WK1'),HarbourCountry2:=aux_countries$ISO2Code[match(HarbourCountry, aux_countries$ISO3Code)]]
							
				# QCA: should yield TRUE otherwise debug on cl and cl_rcg
				nrow(cl[is.na(HarbourCountry) & !is.na(HarbourCountry2),]) == 0
				
		# 'BEN' (Benim - 'BJ' according to UNLOCODE lists), 'ESH' (Western Sahara - 'EH' according to UNLOCODE lists) are not included in the file 'aux_countries' - changed with code lines:
		# 'GM' ('GMB' - Gambia) and 'GW' ('GNB' - Guine Bissau)	
			
			cl[HarbourCountry2 %in% c("GM"), HarbourCountry := "GMB"]
			cl_rcg[HarbourCountry2 %in% c("GM"), HarbourCountry := "GMB"]
			
			cl[HarbourCountry2 %in% c("GW"), HarbourCountry := "GNB"]
			cl_rcg[HarbourCountry2 %in% c("GW"), HarbourCountry := "GNB"]
		
			cl[HarbourCountry2 %in% c("EH"), HarbourCountry := "ESH"]
			cl_rcg[HarbourCountry2 %in% c("EH"), HarbourCountry := "ESH"]
			
			cl[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BEN"]
			cl_rcg[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BEN"]
	...
	# CE 
		...
		# HarbourCountry (ISO3) and HarbourCountry2 (ISO2)			
			...
			# 'BEN' (Benim - 'BJ' according to UNLOCODE lists) and 'ESH' (Western Sahara - 'EH' according to UNLOCODE lists) are not included in the file 'aux_countries' - changed with code lines:
			# 'GM' ('GMB' - Gambia) and 'GW' ('GNB' - Guine Bissau)	
			
			ce[HarbourCountry2 %in% c("GM"), HarbourCountry := "GMB"]
			ce_rcg[HarbourCountry2 %in% c("GM"), HarbourCountry := "GMB"]
			
			ce[HarbourCountry2 %in% c("GW"), HarbourCountry := "GNB"]
			ce_rcg[HarbourCountry2 %in% c("GW"), HarbourCountry := "GNB"]
		
		
			ce[HarbourCountry2 %in% c("EH"), HarbourCountry := "ESH"]
			ce_rcg[HarbourCountry2 %in% c("EH"), HarbourCountry := "ESH"]
			
			ce[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BEN"]
			ce_rcg[HarbourCountry2 %in% c("BJ"), HarbourCountry := "BEN"]
...
# ========================		
	...
	ASFIS_WoRMS_updt <- read.table (file="D:/Documents/PNAB/2022/000_RCG_InterssessionalWork/ISSG_Catch_Effort_Sampling_overviews/ISSG_overviews/RegionalOverviews/data/ASFIS_WoRMS_updt.csv", header=T, sep=";", stringsAsFactors=FALSE)
		...
		cl_rcg[,ISSCAAP:=ASFIS_WoRMS_updt$ISSCAAP[match(cl_rcg$SpeciesAphiaID, ASFIS_WoRMS_updt$AphiaID_accepted)]]
			...
			cl_rcg[Species == "Carassius auratus",ISSCAAP:=11] # Carps, barbels and other cyprinids
		...
		# Adds RCG Catch_group
			...
			aux_spp_categ<-read.table("D:/Documents/PNAB/2022/000_RCG_InterssessionalWork/ISSG_Catch_Effort_Sampling_overviews/ISSG_overviews/RegionalOverviews/data/Table_Species_Categ.txt", header=T, sep="\t")
...
# ========================
	...
	cl[,Catch_group:=factor(Catch_group, levels=sort(unique(Catch_group))),]	## Added 28.04.2021 for keeping same format from last year data_prepared
	...
	cl_rcg[,Catch_group:=factor(Catch_group, levels=sort(unique(Catch_group))),]	## Added 28.04.2021 for keeping same format from last year data_prepared
...
# ================	
	...
	# all countries: 2018-2020
		cl[Year %in% c(2018,2019,2020,2021),sum(OfficialLandingCatchWeight),list(FlagCountry, Year)] [order(FlagCountry, Year)]
		ce[Year %in% c(2018,2019,2020,2021),sum(TripsNumber),list(FlagCountry, Year)] [order(FlagCountry, Year)]
...
# reads aux_stocks dataset
	 #aux_stocks<-read.table("../RegionalOverviews/data/aux_stocks.txt", sep="\t", header=T, colClasses="character", encoding="UTF-8")
	aux_stocks<-read.table("D:/Documents/PNAB/2022/000_RCG_InterssessionalWork/ISSG_Catch_Effort_Sampling_overviews/ISSG_overviews/RegionalOverviews/data/aux_stocks.txt", sep="\t", header=T, colClasses="character", encoding="UTF-8")
	...
	target_rows1a<-cl_rcg$Species=="Nephrops norvegicus" & !cl_rcg$Area %in% c("27.3.a","27.3.a.20","27.3.a.21")
	
	cl$ID[target_rows1]<-paste(cl$SpeciesAphiaID[target_rows1], cl$StatisticalRectangle[target_rows1])
	cl_rcg$ID[target_rows1a]<-paste(cl_rcg$SpeciesAphiaID[target_rows1a], cl_rcg$StatisticalRectangle[target_rows1a])
	...
	target_rows2a<-cl_rcg$Species=="Ammodytes" & !is.na(cl_rcg$StatisticalRectangle) & cl_rcg$StatisticalRectangle %in% aux_stocks$area[aux_stocks$Code %in% c("san.sa.1r","san.sa.2r","san.sa.3r","san.sa.4","san.sa.5r","san.sa.6", "san.sa.7r")]
	
	cl$ID[target_rows2]<-paste(cl$SpeciesAphiaID[target_rows2], cl$StatisticalRectangle[target_rows2])
	cl_rcg$ID[target_rows2a]<-paste(cl_rcg$SpeciesAphiaID[target_rows2a], cl_rcg$StatisticalRectangle[target_rows2a])
	
	target_rows3<-cl$Species=="Pandalus borealis" & cl$Area %in% c("27.4","27.4.a")
	target_rows3a<-cl_rcg$Species=="Pandalus borealis" & cl_rcg$Area %in% c("27.4","27.4.a")
	
	cl$ID[target_rows3]<-paste(cl$SpeciesAphiaID[target_rows3], cl$StatisticalRectangle[target_rows3])
	cl_rcg$ID[target_rows3a]<-paste(cl_rcg$SpeciesAphiaID[target_rows3a], cl_rcg$StatisticalRectangle[target_rows3a])
	...
	target_rows4a<-cl_rcg$Species=="Gadus morhua" & cl_rcg$Area %in% c("27.1","27.2","27.1.b","27.2.a","27.2.a.1", "27.2.a.2","27.2.b","27.2.b.2") & !is.na(cl_rcg$StatisticalRectangle)
	
	cl$ID[target_rows4]<-paste(cl$SpeciesAphiaID[target_rows4], cl$StatisticalRectangle[target_rows4])
	cl_rcg$ID[target_rows4a]<-paste(cl_rcg$SpeciesAphiaID[target_rows4a], cl_rcg$StatisticalRectangle[target_rows4a])
	...
	cl_rcg$ID[!(target_rows1a | target_rows2a | target_rows3a | target_rows4a)]<-paste(cl_rcg$SpeciesAphiaID[!(target_rows1a | target_rows2a | target_rows3a | target_rows4a)], cl_rcg$Area[!(target_rows1a | target_rows2a | target_rows3a | target_rows4a)])
	...
	cl_rcg[, stockNew:=aux_stocks$Code[match(cl_rcg$ID, aux_stocks$ID)]]   
	table(cl_rcg$stockNew)
	

	cl[!is.na(stockNew) & Year==2021,list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, stockNew)][order(-V1),]
	cl_rcg[!is.na(stockNew) & Year==2021,list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, stockNew)][order(-V1),]
	
	cl[!is.na(stockNew) & Year==2021,list(V1=sum(OfficialLandingCatchWeight)), list(stockNew)][order(-V1),][grepl(stockNew, pat="cod.27.1-2"),]
	cl_rcg[!is.na(stockNew) & Year==2021,list(V1=sum(OfficialLandingCatchWeight)), list(stockNew)][order(-V1),][grepl(stockNew, pat="cod.27.1-2"),]
	
	cl[grepl(Area, pat="27.") & Year==2021,list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)), list(Species)]
	cl_rcg[grepl(Area, pat="27.") & Year==2021,list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)), list(Species)]
	
	# species with stocks
	target_species<-unique(cl$Species[!is.na(cl$stockNew)])
		
	out<-cl[grepl(Area, pat="27.") & Year==2021 & Species %in% target_species, list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=round(sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)*100,1), areasWithoutStock=paste(sort(unique(Area[is.na(stockNew)])), collapse=","), stocksPresent=paste(sort(unique(stockNew[!is.na(stockNew)])), collapse=",")), list(Species)]
	...
	target_species<-unique(cl_rcg$Species[!is.na(cl_rcg$stockNew)])
		
	out<-cl_rcg[grepl(Area, pat="27.") & Year==2021 & Species %in% target_species, list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=round(sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)*100,1), areasWithoutStock=paste(sort(unique(Area[is.na(stockNew)])), collapse=","), stocksPresent=paste(sort(unique(stockNew[!is.na(stockNew)])), collapse=",")), list(Species)]
	out1<-cl_rcg[grepl(Area, pat="27.") & Year==2021 & Species %in% target_species, list(withStock=sum(OfficialLandingCatchWeight[!is.na(stockNew)]), withoutStock=sum(OfficialLandingCatchWeight[is.na(stockNew)]), propWithoutStock=round(sum(OfficialLandingCatchWeight[is.na(stockNew)])/sum(OfficialLandingCatchWeight)*100,1), areasWithoutStock=paste(sort(unique(Area[is.na(stockNew)])), collapse=","), stocksPresent=paste(sort(unique(stockNew[!is.na(stockNew)])), collapse=",")), list(FlagCountry, Species)]
		
	xlsx::write.xlsx(out, file="check_stocks_updt4.xlsx")
	xlsx::write.xlsx(out1, file="check_stocks_updt4.xlsx", sheetName="Sheet2", append=T)	
		
	
	# some subsisting NAs may be due to absence of rectangles
	cl[grepl(Area, pat="27.") & Year==2021,,]
	cl[is.na(stockNew) & Year==2021 & FlagCountry=="ESP" & Species ==  "Gadus morhua",]
	
	head(cl[grepl(Area, pat="27.") & is.na(stockNew) & Year==2021 & FlagCountry=="SWE",list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, Area, stockNew)][order(-V1),],10)
	cl[grepl(Area, pat="27.") & is.na(stockNew) & Year==2021 & FlagCountry=="ESP",list(V1=sum(OfficialLandingCatchWeight)), list(FlagCountry, Species, Area, stockNew)][order(-V1),]

	cl$ID <- NULL
	cl_rcg$ID <- NULL
	
	# cl <- cl[,c(1:34,40:42,35:39)]
	# cl_rcg <- cl_rcg[,c(1:34,40:42,35:39)]
	# ce <- ce[,c(1:26,29,27,28)]
	# ce_rcg <- ce_rcg[,c(1:26,29,27,28)]
