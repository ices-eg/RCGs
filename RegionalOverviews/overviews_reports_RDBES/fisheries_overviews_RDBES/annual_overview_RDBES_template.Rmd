...
# functions and packages
	...
	fleetreg<-data.table()
		...
		# based on full history
		 ...
		 dir<-paste("../../data/fleet_reg/output/", year,"/", sep = "") 
	...
	for (ctry in c("BEL","DEU","DNK","ESP","EST","FIN","FRA","IRL","LTU","LVA","NLD","POL","PRT","SWE")) # no data for "GBR"
		...
		fleetreg<-rbind(fleetreg,fread(paste(dir,ctry,"_export.csv", sep = ""), sep = ";",stringsAsFactors=FALSE, verbose=FALSE))
	 		...
	 		# subsets data - need to decide the number of years to include in the summary table... used 5 years for now.
    ...
    fleetreg$V2021<-0
		...
		df2023<-as.data.frame(fleetreg[V2023==20230101,])
		df2023$Status_date<-2023
		df2023<-df2023[target_cols]

    df2022<-as.data.frame(fleetreg[V2022==20220101,])
		df2022$Status_date<-2022
		df2022<-df2022[target_cols]
    ...
    df2021<-as.data.frame(fleetreg[V2021==20210101,])
		...
		fleetreg<-as.data.table(rbind(df2023, df2022, df2021, df2020, df2019))
