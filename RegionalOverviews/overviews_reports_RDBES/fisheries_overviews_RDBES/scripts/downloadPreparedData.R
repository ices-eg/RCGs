...
if(downloadDataFromSP == 1){
  ...
  cat("\n")
  cat("\n")
  cat(paste("Please note that the downloaded data originate from a folder labelled with date:", downloadDataFromSPDate)) # Ideally we should be able to download latest prepared data each time. To do this, the function has to be able to list files at the SP url. 
  cat("\n")
  ...
  dir_output_all <- paste("RegionalOverviews/data_RDBES/002_prepared/", format(Sys.time(), "%Y%m%d"), sep="")
  dir_output_rcg <- paste("RegionalOverviews/data_RDBES/002_prepared/", format(Sys.time(), "%Y%m%d"), "/", regionSelected, sep="")
