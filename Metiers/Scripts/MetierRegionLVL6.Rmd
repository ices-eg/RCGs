...
output: 
  html_document:
    ...
    toc_depth: 4
params:
  region: North Atlantic # Select between - North Sea and Eastern Artic; Baltic Sea; North Atlantic
  year: 2021 # select the year of the data in the report
knit: (function(inputFile, encoding) {
  rmarkdown::render(inputFile,
  encoding = encoding, 
  output_file = file.path(dirname(inputFile), "NAtl.html"))}) # <--- Change region here
---
...
This report presents the description of the main metiers in the `r params$region`. The data used to produce the report is from the 'Commercial Landings' (CL) and 'Commercial Effort' (CE) tables submitted to the RDBES in `r params$year`. The description of the metiers includes information on official landings (weight and value) and official effort (number of fishing days and number of trips), by country, metier and vessel size range. The CL information presented in this report refers only to the catch category for the landings ('Lan'), ignoring the other possible options (e.g. 'BMS' and 'RegDis').
The information is displayed in an hierarchical mode, from the lowest (level 4) to the highest (level 6) metier. The top 10 metiers at level 4 are selected according to the official effort in number of fishing days. The metiers at levels 5 and 6 presented in this report are the ones that have more than 5% of the total fishing days within the previous metier level. 
It is important to note that if official landing weight was considered for selecting the main metiers (instead of the effort), probably other important metiers might also be included in these descriptions.

## All metier level 4 

The landings and effort overview of all metiers at level 4 present in the `r params$region` are displayed in the set of figures below. 
...
create_section4 <- function(met4, CEAREA, CLAREA) {
  ...
  # Header
  pander::pandoc.header(met4, level = 3)
...
create_section5 <- function(met5, CEAREA4, CLAREA4) {
   ...
   # Header
   pander::pandoc.header(met5, level = 4)
      pandoc.p("Summary table with information on the official landings and fishing effort of all the metiers at level 5, by flag country.")
