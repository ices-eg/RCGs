...
T2_1$Comment_0[round(T2_1$landings,0) == 0 & !is.na(T2_1$landings)] <- paste0("National landings rounded to 0 - TRUE mean = ", round(T2_1$landings[round(T2_1$landings,0) == 0 & !is.na(T2_1$landings)], digits = 3))
...
T2_1$coveredLength[T2_1$RFMO %in% c('ICCAT','IOTC','WCPFC') & !(T2_1$landings %in% c("None"))] <- 'Y'
T2_1$thresh[T2_1$RFMO %in% c('ICCAT','IOTC','WCPFC') & !(T2_1$landings %in% c("None"))] <- 'None'

T2_1$thresh[T2_1$spp == 'Anguilla anguilla' & !(T2_1$landings %in% c("None"))] <- 'None'
T2_1$coveredLength[T2_1$spp == 'Anguilla anguilla' & !(T2_1$landings %in% c("None"))] <- 'Y'
T2_1$thresh[T2_1$spp == 'Salmo salar' & !(T2_1$landings %in% c("None"))] <- 'None'
T2_1$coveredLength[T2_1$spp == 'Salmo salar' & !(T2_1$landings %in% c("None"))] <- 'Y'
