...
# 
  ...
  if (whatGoesIn$RCGstats) {
      ...
      if (table_2.1$areaRDB[i] == "ICCAT") {
        RCGS2 <-
        RCGS[RCGS$Scientific_name %in% species[[1]] &
               substr(RCGS$area, 1, 2) %in% c(21, 27, 31, 37, 41, 47, 34, 48), ]
      } 
  ...
  if(RCGS2$source[1]) { sourceEU <- 'RCG agreed statistics'
	} else {
	  sourceEU <- '-'
	}
	if(RCGS2_C$source[1]) { sourceNat <- 'RCG agreed statistics'
	} else {
	  sourceNat <- '-'
	}
...
T2_1$Comment_0[round(T2_1$landings,0) == 0 & !is.na(T2_1$landings)] <- paste0("National landings rounded to 0 and set to None - TRUE mean = ", round(T2_1$landings[round(T2_1$landings,0) == 0 & !is.na(T2_1$landings)], digits = 3))

T2_1$landings <- round(T2_1$landings,0)
T2_1$landings[is.na(T2_1$landings) | T2_1$landings == 0] <- "None"
...
T2_1$thresh[T2_1$landings %in% 'None' & T2_1$TAC %in% 'None']  <- 'None'

# <- T2_1$shareLanding[T2_1$landings %in% 'None' & T2_1$TAC %in% 'None'] <- 'None'
...
T2_1$comments <- paste(T2_1$Comments2, T2_1$Comment_25rule, T2_1$Comment_0, T2_1$comments, sep = "/ ")
...
T2_1$coveredLength[T2_1$RFMO %in% c('ICCAT','IOTC','WCPFC') & !(T2_1$landings %in% c(0, "None"))] <- 'Y'
T2_1$thresh[T2_1$RFMO %in% c('ICCAT','IOTC','WCPFC') & !(T2_1$landings %in% c(0, "None"))] <- 'None'

T2_1$thresh[T2_1$spp == 'Anguilla anguilla' & !(T2_1$landings %in% c(0, "None"))] <- 'None'
T2_1$coveredLength[T2_1$spp == 'Anguilla anguilla' & !(T2_1$landings %in% c(0, "None"))] <- 'Y'
T2_1$thresh[T2_1$spp == 'Salmo salar' & !(T2_1$landings %in% c(0, "None"))] <- 'None'
T2_1$coveredLength[T2_1$spp == 'Salmo salar' & !(T2_1$landings %in% c(0, "None"))] <- 'Y'
