...
# Change country and reference years in params in the YAML (this is so the report can be rendered)
...
output_suffix <- gsub(":", "", paste("_RDB_EUROSTAT", Sys.time(), sep = "_")) # This may be overkill, but nice when testing
...
if (length(rfmos_in_output) == 0) {
  write.table(
    ...
    file = paste(
      pathOut,
      paste(CTRY, "_table2_1_filled", output_suffix, ".csv", sep = ''),
      sep = '/'
    ),
    sep = ';',
    row.names = FALSE,
...
write.table(tabControl, file=paste(pathOut,paste(CTRY,"_table2_1_control", output_suffix, ".csv",sep=''),sep='/'), sep=';',row.names=FALSE, quote=FALSE)
```
