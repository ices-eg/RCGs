...
######################################################
#
pathIn     <-    'D:/Grupy robocze/RCG/fishnCo/NWP table 2.1'
...
```{r warning=FALSE}
  ...
  setwd('.')
  source(paste(pathIn,'Filling Table 21 functions.r',sep='/'))
```
...
if (whatGoesIn$RCGstats) RCG <- loadRCG("D:\\Grupy robocze\\RCG\\fishnCo\\NWP table 2.1")
```
...
table_2.1 <- read.csv(paste(pathIn,'EUMAP_Table_2_1_Linkage_EUROSTAT and EC_TAC_version_2021_final.csv',sep='/'), sep=';', header=TRUE, as.is=TRUE, encoding='UTF-8') #New
```
...
# 
  ...
  if (whatGoesIn$RCGstats) {
      ...
      reg <- strsplit(as.character(table_2.1$areaBis[i]), split = ',')
      RCGS2 <-
        RCGS[RCGS$Scientific_name %in% table_2.1$latinName[i] &
