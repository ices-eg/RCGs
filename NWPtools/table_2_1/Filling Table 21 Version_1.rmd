...
output_suffix <- gsub(":", "", paste("_RDB_EUROSTAT", Sys.time(), sep = "_"))

# whatGoesIn - If EUROSTAT = T & RCGstats = T, then the resulting _control will have both stats, but the _filled will be based on RCGstats
...
table_2.1 <- read.csv(paste(pathIn,'EUMAP_Table_2_1_Linkage_EUROSTAT_RDB and EC_TAC.csv',sep='/'), sep=';', header=TRUE, as.is=TRUE, encoding='UTF-8') #New
	...
	if(RCGS2_C$source[1]) { sourceNat <- 'RCG'
      ...
      if (sum(is.na(RCGS2_C$MOY)) < nrow(RCGS2_C)) {
        ...
        T1$shareLanding <- 0
      	...
      	tbC1 <- data.frame(spp=table_2.1$latinName[i], geo=rep(CTRY, each=3), area=table_2.1$area[i], year=refYears,
