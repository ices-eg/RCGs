...
# whatGoesIn - If EUROSTAT = T & RCGstats = T, then the resulting _control will have both stats, but the _filled will be based on RCGstats due to the ordering in the production
...
)  # Flag to inform on which source to be used (FIDES=FALSE means the system will use a public DB instead)
...
# -----------------------------------------------------------------------
  if (whatGoesIn$FIDEScommon) {
    ...
    ind.ct <- NULL
      ...
      T1$Comments <- NA # Dette skal ændres ......
    ...
    T21$thresh[T21$TAC < .1 & T21$TAC > 0] <- 'TAC < 10%'  #rule (a)
    T21$thresh[is.na(T21$TAC) &
