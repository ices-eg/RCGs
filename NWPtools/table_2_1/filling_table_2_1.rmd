...
params:
  ...
  FIDEScommon: TRUE
  RFMO: "ICES,NAFO"
  output_suffix: "_1820_test_parms"
...
# Settings
## How
All setting are done in the YAML in the R markdown

**Reference years:**

Controlled with ref_years_min and ref_year_max. The script needs 3 years to run due to present way of coding

**Country of interest:**

Contolled by ctry. Use 2-alpha country

**Sources of landings statistics:**

Controlled with EUROSTAT, RCGstats and NationalStats. If e.g. EUROSTAT = TRUE & RCGstats = TRUE, then the resulting _control will have both stats, but the _filled will be based on RCGstats due to the ordering in the script. RCGstats is considered confidential data and can not be shared at GitHub

**Source of TAC / quota:**

Controlled with FIDES or FIDEScommon. FIDES is a national extract from FIDES including a country and EEC, if needed ask Joel for an example. FIDEScommon is a RCG extract from FIDES including all EU countries. These are both confidential. If FIDES=FALSE and FIDEScommon = FALSE, then the system will use a public source instead (see /input/Record of European TACs.xlsx)

**RFMO's to include in output:**
Controlled by RFMO - only subset the _filled not the _control. RCGstats are only reliable in c("ICES", "NAFO") and have only been tested in these. Use "" for all | a list of RFMO's eg. "ICES,NAFO"

**Naming of output files:**
Controlled by output_suffix. Mainly added for the sake of testing. If no need then use ""

**Paths:**
Controlled by pathIn and pathInConf.  latter holds the confidential data e.g. 
...
refYears   <-    c(params$ref_years_min:params$ref_years_max)
CTRY       <-    params$ctry
...
# output_suffix <- gsub(":", "", paste("_RDB_EUROSTAT_new_rdbdata", Sys.time(), sep = "_")) # This may be overkill, but nice when testing


rfmos_in_output <- strsplit(gsub(" ", "", params$RFMO), split = (","))[[1]]
...
if (whatGoesIn$FIDEScommon) TAC <- read.csv(paste0(pathIn, "/confidential_data/Quotas2018-2020.csv"), sep=";", header=TRUE)
...
if (length(rfmos_in_output) == 0) {
  write.table(
    ...
    file = paste(
      pathOut,
      paste(CTRY, "_table_2_1_filled", output_suffix, ".csv", sep = ''),
      sep = '/'
    ),
    sep = ';',
    row.names = FALSE,
...
write.table(tabControl, file=paste(pathOut,paste(CTRY,"_table_2_1_control", output_suffix, ".csv",sep=''),sep='/'), sep=';',row.names=FALSE, quote=FALSE)
```
