...
input_path_dnk <- "Q:/scientific-projects/eu-data-collection/Work_Plan/2020/scripts/gits/RCGs/NWPtools/personal_folders/dnk"
...
COD <- read.table(file.path(input_path_common, 'patch_codIIIa_20191009.txt'), sep = '\t', header = TRUE, as.is = TRUE)
...
table1A <- read.csv(file.path(input_path_common,'EUMAP_Table1A_Linkage_EUROSTAT and EC_TAC.csv'), sep=';',header=TRUE, as.is=TRUE)
```
...
write.table(T1B[ind,], file=paste(output_path, CTRY,'_table1A_filled_common.csv',sep=''), sep=';',row.names=FALSE, quote=FALSE)
```
